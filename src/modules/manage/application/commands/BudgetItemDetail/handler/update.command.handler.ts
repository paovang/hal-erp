...
import { UpdateBudgetItemDetailCommand } from '../update.command';
...
export class UpdateBudgetItemDetailCommandHandler
  implements
    IQueryHandler<
      ...
      ResponseResult<BudgetItemDetailEntity>
    >
{
  constructor(
  ...
  async execute(query: UpdateBudgetItemDetailCommand): Promise<any> {
    return await this._transactionManagerService.runInTransaction(
      ...
      async (manager) => {
        ...
        await mapToEntity.initializeUpdateSetId(
          new BudgetItemDetailId(query.id),
        );
        await mapToEntity.validateExistingIdForUpdate();
        const result = await this._write.update(mapToEntity, manager);
