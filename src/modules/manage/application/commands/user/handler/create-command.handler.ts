  ...
  implements IQueryHandler<CreateCommand, ResponseResult<UserEntity>>
{
  ...
  async execute(
    ...
    await _checkColumnDuplicate('email', query.dto.email, query.manager, 'Email already exists');
    await _checkColumnDuplicate('tel', query.dto.tel, query.manager, 'Tel already exists');
