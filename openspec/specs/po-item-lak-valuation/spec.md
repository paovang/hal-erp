# po-item-lak-valuation Specification

## Purpose

Persist a LAK-equivalent valuation (including VAT) on every Purchase Order (PO) item so reporting, exports, and downstream consumers can read both the LAK total and LAK VAT directly without re-resolving exchange rates. Each PO item carries the exchange rate that was applied (`rate`), the LAK-denominated total (`total_in_lak`), and the LAK-denominated VAT (`vat_in_lak`) alongside its native-currency `total`, `vat`, and `total_with_vat`.
