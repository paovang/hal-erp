# user-name-fields Specification

## Purpose

Persist a user's given name as separate `firstName` and `lastName` properties on the user aggregate so the API can create, update, return, and search users by their human names independently of the existing `username`, `email`, and `tel` identifiers. Legacy user rows created before this capability MAY have null names; all new and updated users MUST supply both values through the API.
